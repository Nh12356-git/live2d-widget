...
import { LocalModelManager, LocalConfig, LocalModelList } from './localmodel.js';
 ...
 */
async function loadWidget(config: LocalConfig) {
  ...
  const model = await LocalModelManager.initCheck(config, models);
 ...
 */
function initWidget(config: string | LocalConfig) {
  ...
  toggle?.addEventListener('click', () => {
    ...
    if (toggle?.getAttribute('first-time')) {
      loadWidget(config as LocalConfig);
