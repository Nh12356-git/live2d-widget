...
export class LAppModel extends CubismUserModel {
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    const setupLayout = (): void => {
      ...
      // 模型整体向上平移至合适位置（画布坐标，模型高度=2.0，正值向上）
      this._modelMatrix.translate(0.0, 0.25);
  ...
  public doDraw(): void {
    ...
    // キャンバスサイズを渡す（右下を原点にオフセット）
    const canvas = this._subdelegate.getCanvas();

    // モデルをキャンバス内で相対移動させるオフセット（CSSピクセル単位）
    // 例: { x: -20, y: 30 } は左に20px・下に30px移動
    // WebGL viewport の y は下方向が負のため、y は反転して適用する
    const offset = (window as any).__live2dModelOffset;
    const cssW = canvas.offsetWidth || canvas.width;
    const cssH = canvas.offsetHeight || canvas.height;
    const dx = offset ? (offset.x * canvas.width) / cssW : 0;
    const dy = offset ? -(offset.y * canvas.height) / cssH : 0;

    const viewport: number[] = [
      Math.floor(canvas.width * 0.15 + dx),   // x: 右移 + 左オフセット
      Math.floor(dy),                         // y: オフセット
      canvas.width,                           // width: 不变
      canvas.height                           // height: 不变
    ];
